...
echo "create an initial configuration file with instance id $instanceid"
cat > $CONFIGFILE <<EOF;
<?php
\$CONFIG = array (
  'datadirectory' => '/data',
...
echo "Authorized host is $DOMAIN"
sed -i "s/localhost/$DOMAIN/g" $CONFIGFILE

echo "Add nextcloud to authorized hosts"
sed -i "s/0 => '$DOMAIN',/0 => '$DOMAIN', 1 => 'nextcloud',/" $CONFIGFILE
